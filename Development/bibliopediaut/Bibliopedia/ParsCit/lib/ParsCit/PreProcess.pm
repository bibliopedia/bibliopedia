...
# Added "notes" - mlw <mikewidner@gmail.com> 29 Apr 2010
#    while ($text =~ m/\b(References?|REFERENCES?|Bibliography|BIBLIOGRAPHY|References?\s+and\s+Notes?|References?\s+Cited|REFERENCE?\s+CITED|REFERENCES?\s+AND\s+NOTES?):?\s*\n+/sg) {
    while ($text =~ m/\b(Notes?|NOTES?|References?|REFERENCES?|Bibliography|BIBLIOGRAPHY|References?\s+and\s+Notes?|References?\s+Cited|REFERENCES?\s+CITED|REFERENCES?\s+AND\s+NOTES?):?\s*\n+/sg) {
	$bodyText = substr $text, 0, pos $text;
	$citeText = substr $text, pos $text unless (pos $text < 1);
    }
    
# removed check for length of citations, enables us to give ParsCit straight citation text for extraction
# mlw - <mikewidner@gmail.com> 29 Apr 2010
